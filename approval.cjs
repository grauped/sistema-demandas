...
const fields=['instructorId','instructorSnapshot','department','course','group','shift','discipline','type','hoursUnits','hourRateCents','amountCents','startDate','endDate','referenceDate','requestDate','requester','notes','lessonCount','cancelled'];
...
function signature(item){return JSON.stringify(fields.map(key=>stable(item[key]??null)));}
